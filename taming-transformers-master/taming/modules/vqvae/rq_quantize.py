...
class RQBottleneck(nn.Module):
    ...
    def quantize(self, x):
        ...
        for i in range(self.code_shape[-1]):
            ...
            # print(quant.shape, residual_feature.shape)
    ...
    def forward(self, x):
        ...
        x_reshaped = rearrange(x_reshaped, 'b c h w -> b h w c').contiguous()
        ...
        quants_trunc = rearrange(quants_trunc, 'b h w c -> b c h w').contiguous()
