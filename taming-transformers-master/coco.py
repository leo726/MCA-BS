...
from vqvae import VectorQuantize as MultiheadVQ
...
class Attention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        q = self.to_q(x)    # q: torch.Size([1, 256, 64])
        context = default(context, x)
        k, v = self.to_kv(context).chunk(2, dim=-1) # k: torch.Size([1, 1024, 64]) v: torch.Size([1, 1024, 64])

        q, k, v = map(lambda t: rearrange(t, "b n (h d) -> (b h) n d", h=h), (q, k, v))

        sim = torch.einsum("b i d, b j d -> b i j", q, k) * self.scale # sim: torch.Size([1, 256, 1024])

        # add mask
        # mask = []
        # if exists(self.sparse_topk) and self.sparse_topk < sim.shape[-1]:
        #     top, _ = sim.topk(self.sparse_topk, dim = -1)
        #     vk = rearrange(top[..., -1], '... -> ... 1')
        #     sparse_topk_mask = sim < vk
        #     mask.append(sparse_topk_mask)
...
class CodebookAttention(nn.Module):
    ...
    def forward(self, input, codebook):
...
if __name__ == "__main__":
    codebookAttention = CodebookAttention(
                 codebook_dim = 256,
                 depth = 1,
                 num_latents = 256,
                 latent_dim = 256,
    ...
    print(codebookAttention)
    # codebook = torch.rand(4,8,1024,32)
    # codebook = torch.rand(1,1024,256)
    quantize = MultiheadVQ(dim = 256,codebook_dim = 32,                  # a number of papers have shown smaller codebook dimension to be acceptable
                            heads = 8,                          # number of heads to vector quantize, codebook shared across all heads
                            separate_codebook_per_head = True,  # whether to have a separate codebook per head. False would mean 1 shared codebook
                            codebook_size = 1024, # 8196
                            accept_image_fmap = True
                        )
    codebook = quantize.codebook.reshape(1,1024,256)
    x = torch.rand(256,256)
    x1 = codebookAttention(x,codebook)