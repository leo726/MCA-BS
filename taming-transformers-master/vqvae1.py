...
class VectorQuantize(nn.Module):
    ...
    def forward(
        ...
        # x=x.reshape(8,256,32)
        x=x.reshape(8,1024,32)
        x1 = self.codebookAttention(x,self.codebook)
        if self.training:
            x1 = x + (x1 - x).detach()

        # x1 = x1.reshape(8,1,256,32)
        x1 = x1.reshape(8,1,1024, 32)

        if is_multiheaded:
            if self.separate_codebook_per_head:
                x1 = rearrange(x1, 'h b n d -> b n (h d)', h = heads)
            else:
                x1 = rearrange(x1, '1 (b h) n d -> b n (h d)', h = heads)

        # x1 = self.project_out(x1)

        if need_transpose:
            x1 = rearrange(x1, 'b n d -> b d n')

        # cut 32x32    
        # if self.accept_image_fmap:
        #     x1 = rearrange(x1, 'b (h w) c -> b c h w', h = height, w = width)
...
if __name__ == "__main__":
    ...
    # img_fmap = torch.randn(1, 256, 16, 16)
    # x1 = vq(img_fmap)
